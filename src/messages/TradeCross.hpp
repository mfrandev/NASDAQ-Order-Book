...
TradeCross parseTradeCrossBody(const char data[MAX_BUFFER_SIZE]) {
    size_t offset = 0;
    uint64_t crossShares = toHostEndianUpTo64(&data[offset], MessageFieldSizes::SHARES_IN_TRADE_MESSAGE_SIZE);
    ...
    return TradeCross{crossType, crossPrice, crossShares, matchNumber};
}
